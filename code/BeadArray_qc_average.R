# To use the illuminaBackground instead of medianBackground which will a very close emulation of that used by Illumina
...
raw_qc_average_dir <- toString(args[2])

# dir.create(paste(getwd(), "/raw_qc_average/", sep = ""), showWarnings = TRUE, recursive = FALSE, mode = "0777")
...
tiff.Files = list.files(pattern = "*.tif")
...
for (i in 1:length(array.names)) {
  ...
  setwd(paste(getwd(), "/raw_qc_average/", sep = ""))
...
array.qcvalues = matrix(, nrow = length(array.names), ncol = 60)

# BASH performs three types of artefact detection in the style of the affmetrix-oriented Harsh-light package: Compact analysis identiffes large clusters of outliers, where each outlying bead must be an immediate neighbour of another outlier; Diffuse analysis finds regions that contain more outliers than would be anticipated by chance, and Extended analysis looks for chip-wide variation, such as a consistent gradient effect
...
for (j in 1:length(array.names)) {
  ...
  # HULK uses information about neighbouring beads, but rather than mask them out as in BASH, it adjusts the log-intensities by the weighted average of residual values within a local neigh-bourhood
...
array.qcvalues[, 34] = as.matrix(dim(datasumm))[1]
array.qcvalues[, 35] = as.matrix(dim(datasumm))[2]
...
for (l in 1:length(array.names)) {
  ...
  array.qcvalues[l, 58] = n.count
  array.qcvalues[l, 59] = as.matrix(dim(datasumm))[1]
  array.qcvalues[l, 60] = n.count/as.matrix(dim(datasumm))[1]
...
# To write an entire QC talbe
write.table(array.qcvalues, file = paste(sapply(strsplit(tiff.Files[1], "_"), "[", 1), "raw_qc_average_details.txt", sep = "_"), append = FALSE, quote = FALSE, sep = "\t", eol = "\n", na = "NA", dec = ".", row.names = array.names, col.names = FALSE)
