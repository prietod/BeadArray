...
# The median and standard errors for each bead type are reported
...
# All observations are extracted, transformed and then grouped together according to their ArrayAddressID. Outliers are removed and the median and standard errors of the remaining beads are calculated
# The default options of summarize apply a un-log transformation, remove outliers using the Illumina 3 M.A.D cut-off and report the median and standard errors for each bead type
grnchannel.unlogged <- new("illuminaChannel", transFun = greenChannelTransform, outlierFun = illuminaOutlierMethod, exprFun = function(x) median(x, na.rm = TRUE),  varFun = function(x) mad(x, na.rm = TRUE), channelName = "G")
