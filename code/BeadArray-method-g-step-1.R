...
logGreenChannelTransformGrnF <- function(BLData, array, what) {
...
for (j in 1:length(array.names)) {
  ...
  BASHoutput <- suppressMessages(BASH(beadarray.data, array = j, transFun = logGreenChannelTransformGrnF, outlierFun = illuminaOutlierMethod, compact = TRUE, diffuse = TRUE, extended = TRUE, bgcorr = "none"))
  ...
  HULKoutput <- suppressMessages(HULK(beadarray.data, array = j, weightName = "wts", transFun = logGreenChannelTransformGrnF, outlierFun = illuminaOutlierMethod))
