...
data <- readIllumina(dir=data_dir, useImages=TRUE, illuminaAnnotation="Humanv4")
...
TIFF_Files = list.files(path=data_dir, pattern="*.tif")
...
for (i in 1:length(TIFF_Files)) {
  ...
  TIFF <- readTIFF(file.path(data_dir, TIFF_Files[i]))
