...
donor.data <- as.matrix(read.table("Donor_2_Pearson.txt", header = TRUE, sep = "\t"))
...
pdf(file = "Pearson_correlation_coefficient_donor_2.pdf", width = 11, height = 8.5)

# Graph autos using y axis that ranges from 0 to max.y
# Turn off axes and annotations (axis labels) so it can be specified
plot(as.numeric(donor.data[, "Pearson.90.r.percentage"]), type = "o", col = plot.colors[1], ylim = c(0.7, max.y), axes = FALSE, ann = FALSE)
...
lines(as.numeric(donor.data[, "Pearson.85.r.percentage"]), type = "o", pch = 22, lty = 2, col = plot.colors[2])

# Graph values with green dotted line and diamond points
lines(as.numeric(donor.data[, "Pearson.80.r.percentage"]), type = "o", pch = 23, lty = 3, col = plot.colors[3])
...
legend(0.7, 0.75, c("Pearson: r cutoff = 0.90", "Pearson: r cutoff = 0.85", "Pearson: r cutoff = 0.80"), cex = 0.8, col = plot.colors, pch = 21:23, lty = 1:3)
