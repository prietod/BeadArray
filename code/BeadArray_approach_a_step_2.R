...
# In beadarray
# Background correction using normal-exponential convolution using negative controls (neqc)
...
# To store gene expression values for all the array
array.normalized.lumi <- matrix(, nrow = dim(as.matrix(data.gene.lumi.n))[1], ncol = (dim(as.matrix(data.gene.lumi.n))[2] + 2))

# To store Illumina IDs information
array.normalized.lumi[, 1] = row.names(data.gene.lumi.n)
...
# To define column names
colnames(array.normalized.lumi) <- c("Probe_ID", "Entrez_ID", colnames(data.gene.lumi.n))

# To retrieve quality information and verify that probes annotated as "Bad" or "No Match" generally have lower signal
ids.arrays.lumi <- as.character(rownames(data.gene.lumi.n))
qual.lumi <- unlist(mget(ids.arrays.lumi, illuminaHumanv4PROBEQUALITY, ifnotfound = NA))

# To store normalized gene expression data for only high quality probes
rem.lumi <- qual.lumi == "No match" | qual.lumi == "Bad"
data.gene.lumi.n.filt <- data.gene.lumi.n[!rem.lumi, ]

# To store gene expression values for all the array
array.normalized.filt.lumi <- matrix(, nrow = dim(as.matrix(data.gene.lumi.n.filt))[1], ncol = (dim(as.matrix(data.gene.lumi.n.filt))[2] + 2))

# To store Illumina IDs information
array.normalized.filt.lumi[, 1] = row.names(data.gene.lumi.n.filt)
...
# To define column names
colnames(array.normalized.filt.lumi) <- c("Probe_ID", "Entrez_ID", colnames(data.gene.lumi.n.filt))

# To gene expression values
array.normalized.lumi[, 3:dim(as.matrix(array.normalized.lumi))[2]] = exprs(data.gene.lumi.n)[, 1:dim(as.matrix(data.gene.lumi.n))[2]]
array.normalized.filt.lumi[, 3:dim(as.matrix(array.normalized.filt.lumi))[2]] = exprs(data.gene.lumi.n.filt)[, 1:dim(as.matrix(data.gene.lumi.n.filt))[2]]
...
# To get the donors information
# To run a for loop for all the samples
for (donor.1.info.lumi in 1:dim(as.matrix(data.gene.lumi.n))[2]) {
...
# To get the donors information
# To run a for loop for all the samples
for (donor.2.info.lumi in 1:dim(as.matrix(data.gene.lumi.n))[2]) {
...
for (donor.data.lumi in 1:dim(as.matrix(data.gene.lumi.n))[2]) {

  # To filter out samples based on donor information
  if (as.character(colnames(data.gene.lumi.n)[donor.data.lumi]) %in% as.character(as.matrix(donor.1.lumi))) {
    ...
    n_donor.1.col.names.lumi = c(n_donor.1.col.names.lumi, as.character(colnames(data.gene.lumi.n)[donor.data.lumi]))
  }

  else if (as.character(colnames(data.gene.lumi.n)[donor.data.lumi]) %in% as.character(as.matrix(donor.2.lumi))) {
    ...
    n_donor.2.col.names.lumi = c(n_donor.2.col.names.lumi, as.character(colnames(data.gene.lumi.n)[donor.data.lumi]))
  }
    ...
    n_wo.donors.col.names.lumi = c(n_wo.donors.col.names.lumi, as.character(colnames(data.gene.lumi.n)[donor.data.lumi]))
  }

}

for (donor.data.filt.lumi in 1:dim(as.matrix(data.gene.lumi.n.filt))[2]) {

  # To filter out samples based on donor information
  if (as.character(colnames(data.gene.lumi.n.filt)[donor.data.filt.lumi]) %in% as.character(as.matrix(donor.1.lumi))) {
    ...
    n_donor.1.col.names.filt.lumi = c(n_donor.1.col.names.filt.lumi, as.character(colnames(data.gene.lumi.n.filt)[donor.data.filt.lumi]))
  }

  else if (as.character(colnames(data.gene.lumi.n.filt)[donor.data.filt.lumi]) %in% as.character(as.matrix(donor.2.lumi))) {
    ...
    n_donor.2.col.names.filt.lumi = c(n_donor.2.col.names.filt.lumi, as.character(colnames(data.gene.lumi.n.filt)[donor.data.filt.lumi]))
  }
    ...
    n_wo.donors.col.names.filt.lumi = c(n_wo.donors.col.names.filt.lumi, as.character(colnames(data.gene.lumi.n.filt)[donor.data.filt.lumi]))
  }
